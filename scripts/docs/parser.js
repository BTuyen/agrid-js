...
const loadApiPackage = (filePath) => apiExtractor.ApiPackage.loadFromJsonFile(filePath)

const findPostHogClass = (apiPackage, className) =>
    apiPackage.entryPoints[0].members.find(
        (member) => member.kind === apiExtractor.ApiItemKind.Class && member.name === className
    )
...
const findExtraMethods = (apiPackage, extraMethodNames) => {
    ...
    return apiPackage.entryPoints[0].members.filter(
        (member) =>
            (member.kind === apiExtractor.ApiItemKind.Function || member.kind === apiExtractor.ApiItemKind.Class) &&
...
const enhanceTypeWithExample = (type, config) => {
    return config.typeExamples[type.name] ? { ...type, example: config.typeExamples[type.name] } : type
}
...
const composeOutput = (posthogClass, functions, types, config) => ({
    ...
    categories: [
        ...new Set([
            ...
            'Identification',
            'Capture',
            ...functions.map((f) => f.category).filter(Boolean),
        ]),
    ],
})
...
const generateApiSpecs = (config) => {
    ...
    const resolvedTypes = types.resolveTypeDefinitions(apiPackage).map((type) => enhanceTypeWithExample(type, config))
