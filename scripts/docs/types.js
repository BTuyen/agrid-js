...
function resolveTypeDefinitions(apiPackage) {
    ...
    const resolvedTypeDefinitions = resolveObjectTypes(resolveUnionTypes(typeDefinitions))
...
function findTypeByCanonicalReference(apiPackage, canonicalReference) {
    ...
    return apiPackage.entryPoints.flatMap((entryPoint) => entryPoint.members).find((member) => member.name === typeName)
}
