...
const hasDocumentationSummary = (apiItem) => Boolean(apiItem.tsdocComment?.summarySection)

const extractSummaryText = (apiItem) => utils.renderDocNodeToText(apiItem.tsdocComment.summarySection)
...
const getDocComment = (apiItem) =>
    hasDocumentationSummary(apiItem) ? extractSummaryText(apiItem) : NO_DESCRIPTION_AVAILABLE

// for parameter documentation
const findParamBlock = (apiMethod, paramName) => apiMethod.tsdocComment?.params?.tryGetBlockByName(paramName)

const extractParamContent = (paramBlock) => (paramBlock?.content ? utils.renderDocNodeToText(paramBlock.content) : null)
...
const hasRemarks = (apiItem) => Boolean(apiItem.tsdocComment?.remarksBlock)

const extractRemarksContent = (apiItem) => utils.renderDocNodeToText(apiItem.tsdocComment.remarksBlock.content)
 ...
 */
const getRemarks = (apiItem) => (hasRemarks(apiItem) ? extractRemarksContent(apiItem) : null)
