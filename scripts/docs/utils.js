...
const cleanObjectSignature = (signature) => signature.replace(/^\s*{\s*|\s*}\s*$/g, '').trim()
...
const parseObjectTypeSignature = (signature) => {
    ...
    return splitObjectProperties(cleaned).map(parseProperty).filter(isNotNull)
}
...
const isCallbackType = (typeString) =>
    callbackIndicators.some(
        (indicator) => typeString.toLowerCase().includes(indicator.toLowerCase()) || typeString.includes(indicator)
    )
 ...
 */
const generateCallbackExample = (typeString) => (isCallbackType(typeString) ? '() => {}' : null)
 ...
 */
const textToId = (text) => text.toLowerCase().replace(/ /g, '_')
 ...
 */
const formatParameterList = (params) => (params || []).map((p) => `<${p.name}>`).join(', ')
 ...
 */
const hasItems = (array) => array && array.length > 0
