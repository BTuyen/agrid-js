...
import { getAugmentPropertiesFromElement, getDefaultProperties, getPropertiesFromElement, previousElementSibling, } from '../autocapture';
...
export function makeMouseEvent(partialEvent) {
    return { type: 'click', ...partialEvent };
}
export function makeCopyEvent(partialEvent) {
    return { type: 'copy', ...partialEvent };
}
export function makeCutEvent(partialEvent) {
    return { type: 'cut', ...partialEvent };
}
function setWindowTextSelection(s) {
    window.getSelection = () => {
        return {
            toString: () => s,
        };
...
describe('Autocapture system', () => {
    ...
    describe('_captureEvent', () => {
        ...
        ])('autocapture and rageclick testcase: %s', (_title, rageclickConfig, parentClassname, expectedCaptured) => {
        ...
        it('assigns element_selector if htmlelement matches known selectors', () => {
            ...
            autocapture.setElementSelectors(new Set(['#primary_button']));
            const elGrandparent = document.createElement('a');
            elGrandparent.setAttribute('href', 'https://test.com');
            elGrandparent.appendChild(elParent);
            autocapture['_captureEvent'](makeMouseEvent({
                target: elTarget,
            }));
        ...
        it('does not include $click_external_href for same site', () => {
            window.location = new URL('https://www.example.com/location');
            const elTarget = document.createElement('img');
            const elParent = document.createElement('span');
            elParent.appendChild(elTarget);
            const elGrandparent = document.createElement('a');
      ...
      `;
            ...
            const text1 = "Some super duper really long Text with new lines that we'll strip out and also we will want to make this text shorter since it's not likely people really care about text content that's super long and it also takes up more space and bandwidth. Some super d";
      ...
      `;
        ...
        it('returns elementsChain correctly with newlines in css', () => {
            ...
            autocapture['_captureEvent'](e);
            const props1 = beforeSendMock.mock.calls[0][0].properties;
            expect(props1['$elements_chain']).toBe('a.test-class.test-class2.test-class3.test-class4.test-class5:attr__class="test-class test-class2 test-class3 test-class4 test-class5"attr__href="http://test.com"href="http://test.com"nth-child="1"nth-of-type="1";span:nth-child="1"nth-of-type="1"');
        });
    ...
    describe('afterFlagsResponse()', () => {
        ...
        it.each([
            ...
            [true, true, false],
            [true, false, true],
        ])('when client side config is %p and remote opt out is %p - autocapture enabled should be %p', (clientSideOptIn, serverSideOptOut, expected) => {
        ...
        it('should not call _addDomEventHandlders if autocapture is opted out in server config', () => {
            autocapture.onRemoteConfig({ autocapture_opt_out: true });
    ...
    describe('shouldCaptureDomEvent autocapture config', () => {
        it('only capture urls which match the url regex allowlist', () => {
            ...
            window.location = new URL('https://posthog.com/test/matching');
            expect(shouldCaptureDomEvent(button, e, autocapture_config)).toBe(true);
            window.location = new URL('https://posthog.com/docs/not-matching');
        ...
        it('If a user sets dom_event_allowlist and element_allowlist with css_selector_allowlist it us a union so only the dom events on the element allow list with the css selector', () => {
            ...
            events.forEach((event) => {
                ...
                createdElements.forEach((element) => {
                    const expectedCapturable = element.tagName === 'BUTTON' && element.className === 'capturable' && event === 'click';
                    const fakeEvent = makeMouseEvent({
                        target: element,
                        type: event,
                    });
