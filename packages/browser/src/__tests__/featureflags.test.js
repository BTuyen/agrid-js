...
describe('featureflags', () => {
    ...
    beforeEach(() => {
        instance = {
            ...
            _send_request: jest.fn().mockImplementation(({ callback }) => callback({
                statusCode: 200,
                json: {},
            })),
            _onRemoteConfig: jest.fn(),
            reloadFeatureFlags: () => featureFlags.reloadFeatureFlags(),
            _shouldDisableFlags: () => instance.config.advanced_disable_flags || instance.config.advanced_disable_decide || false,
        };
    ...
    it('should warn if /flags endpoint was not hit and no flags exist', () => {
        ...
        expect(window.console.warn).toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', 'isFeatureEnabled for key "beta-feature" failed. Feature flags didn\'t load in time.');
        mockWarn.mockClear();
        expect(featureFlags.getFeatureFlag('beta-feature')).toEqual(undefined);
        expect(window.console.warn).toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', 'getFeatureFlag for key "beta-feature" failed. Feature flags didn\'t load in time.');
    });
    ...
    describe('feature flag overrides', () => {
        ...
        describe('deprecated override method', () => {
            it('supports basic flag overrides with warning behavior', () => {
                ...
                expect(window.console.warn).toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', ' Overriding feature flags!', expect.any(Object));
                // Test suppressed warning behavior
                mockWarn.mockClear();
                featureFlags.override({
                    'alpha-feature-2': false,
                }, { suppressWarning: true });
                expect(window.console.warn).not.toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', ' Overriding feature flags!');
                expect(featureFlags.getFlagVariants()).toEqual({
                    'beta-feature': true,
                    'alpha-feature-2': false,
                });
            ...
            it('shows deprecation warning', () => {
                ...
                expect(window.console.warn).toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', 'override is deprecated. Please use overrideFeatureFlags instead.');
            });
        });
        describe('new overrideFeatureFlags method', () => {
            ...
            it('supports payload overrides', () => {
                ...
                expect(window.console.warn).not.toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', ' Overriding feature flag payloads!');
                ...
                expect(window.console.warn).toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', ' Overriding feature flag payloads!', expect.any(Object));
            });
        ...
        describe('callback behavior', () => {
            ...
            it('triggers callback with feature flag changes', () => {
                ...
                expect(callbackSpy).toHaveBeenCalledWith(['alpha-feature-2'], { 'alpha-feature-2': true }, expect.any(Object));
                ...
                expect(callbackSpy).toHaveBeenCalledWith(['alpha-feature-2'], { 'alpha-feature-2': 'variant-1' }, expect.any(Object));
            });
        });
    });
    describe('flags()', () => {
    ...
    describe('onFeatureFlags', () => {
        beforeEach(() => {
            instance._send_request = jest.fn().mockImplementation(({ callback }) => callback({
                statusCode: 200,
                json: {
                    featureFlags: {
                        first: 'variant-1',
        ...
        it('onFeatureFlags should not be called immediately if feature flags not loaded', () => {
            ...
            featureFlags.onFeatureFlags((flags, variants, errors) => {
                ...
                _flags = flags;
                _variants = variants;
                _error = errors === null || errors === void 0 ? void 0 : errors.errorsLoading;
            });
    ...
    describe('earlyAccessFeatures', () => {
        ...
        it('getEarlyAccessFeatures replaces existing features completely instead of merging', () => {
            ...
            expect(instance.persistence.props.$early_access_features).toEqual([EARLY_ACCESS_FEATURE_FIRST]);
            expect(instance.persistence.props.$early_access_features).not.toContainEqual(expect.objectContaining({ flagKey: 'old-feature' }));
        });
    ...
    describe('when /flags times out or errors out', () => {
        ...
        it('should call onFeatureFlags even when /flags errors out', () => {
            ...
            featureFlags.onFeatureFlags((flags, variants, errors) => {
                ...
                _flags = flags;
                _variants = variants;
                _errors = errors === null || errors === void 0 ? void 0 : errors.errorsLoading;
            });
            expect(called).toEqual(false);
            featureFlags.reloadFeatureFlags();
            jest.runAllTimers();
    ...
    describe('Feature Flag Request ID', () => {
        ...
        it('includes requestId in feature flag called event', () => {
            ...
            expect(instance.capture).toHaveBeenCalledWith('$feature_flag_called', expect.objectContaining({
...
describe('parseFlagsResponse', () => {
    ...
    it('enables multivariate feature flags from /flags?v=2 response', () => {
        ...
        expect(window.console.warn).toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', 'Using an older version of the feature flags endpoint. Please upgrade your PostHog server to the latest version');
    });
    ...
    it('enables feature flags from /flags response (v1 backwards compatibility)', () => {
        ...
        expect(window.console.warn).toHaveBeenCalledWith('[PostHog.js] [FeatureFlags]', 'v1 of the feature flags endpoint is deprecated. Please use the latest version.');
    });
...
describe('filterActiveFeatureFlags', () => {
    ...
    it('should filter flags', () => {
        expect(filterActiveFeatureFlags({
            'flag-1': true,
            'flag-2': false,
            'flag-3': 'variant-1',
        })).toEqual({
...
describe('getRemoteConfigPayload', () => {
    ...
    it('should include evaluation_environments when configured', () => {
        ...
        expect(instance._send_request).toHaveBeenCalledWith(expect.objectContaining({
            method: 'POST',
    ...
    describe('flags_api_host configuration', () => {
        it('should use flags_api_host when configured', () => {
            ...
            const customInstance = {
                ...
                get_distinct_id: () => 'test-distinct-id',
                _send_request: jest.fn(),
                requestRouter: new RequestRouter({ config: apiConfig }),
            };
            ...
            expect(customInstance._send_request).toHaveBeenCalledWith(expect.objectContaining({
                method: 'POST',
