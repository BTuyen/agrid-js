...
describe('Rate Limiter', () => {
    ...
    let persistedBucket = {};
    let mockPostHog;
    const moveTimeForward = (milliseconds) => {
    ...
    describe('client side', () => {
        ...
        it('captures a rate limit event the first time it is rate limited', () => {
            ...
            expect(mockPostHog.capture).toHaveBeenCalledWith('$$client_ingestion_warning', {
                $$client_ingestion_warning_message: 'posthog-js client rate limited. Config is set to 10 events per second and 100 events burst limit.',
            }, {
        ...
        it('does not capture a rate limit event if the persisted config was already rate limited', () => {
            ...
            const newRateLimiter = new RateLimiter(mockPostHog);
            range(200).forEach(() => {
    ...
    describe('server side', () => {
        ...
        it('sets the events retryAfter on checkForLimiting', () => {
            ...
            const expectedRetry = new Date().getTime() + 60000;
