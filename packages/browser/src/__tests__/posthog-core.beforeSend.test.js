...
const editingEventFn = (captureResult) => {
    return {
        ...captureResult,
        properties: {
...
describe('posthog core - before send', () => {
    ...
    const posthogWith = (configOverride) => {
    ...
    it('can reject an event', () => {
        ...
        posthog._send_request.mockClear();
    ...
    it('warned when making arbitrary event invalid', () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(`Event '${eventName}' has no properties after beforeSend function, this is likely an error.`);
    });
    it('logs a warning when rejecting an unsafe to edit event', () => {
        ...
        const randomUnsafeEditableEvent = knownUnsafeEditableEvent[Math.floor(Math.random() * knownUnsafeEditableEvent.length)];
        posthog.capture(randomUnsafeEditableEvent, {}, {});
        expect(mockLogger.warn).toHaveBeenCalledWith(`Event '${randomUnsafeEditableEvent}' was rejected in beforeSend function. This can cause unexpected behavior.`);
    });
});
