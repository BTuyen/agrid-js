...
describe('identify()', () => {
    ...
    beforeEach(() => {
        ...
        const posthog = defaultPostHog().init(token, {
    ...
    describe('invalid id passed', () => {
        it('does not update user', () => {
            ...
            expect(console.error).toHaveBeenCalledWith('[PostHog.js]', 'Unique user id has not been set in posthog.identify');
        });
        it('does not update user when distinct ID is $posthog_cookieless', () => {
            ...
            expect(beforeSendMock).not.toHaveBeenCalled();
            expect(instance.register).not.toHaveBeenCalled();
            expect(console.error).toHaveBeenCalledWith('[PostHog.js]', 'The string "$posthog_cookieless" was set in posthog.identify which indicates an error. This ID is only used as a sentinel value.');
        });
    });
    describe('reloading feature flags', () => {
        ...
        it('reloads feature flags if identity does not change but properties do', () => {
            ...
            expect(instance.featureFlags.setAnonymousDistinctId).not.toHaveBeenCalled();
            expect(instance.featureFlags.reloadFeatureFlags).not.toHaveBeenCalled();
            expect(instance.featureFlags.setPersonPropertiesForFlags).toHaveBeenCalledWith({ email: 'john@example.com', howOftenAmISet: 'once!' }, true);
        });
        it('reloads feature flags if identity and properties change', () => {
            instance.identify('a-new-id', { email: 'john@example.com' }, { howOftenAmISet: 'once!' });
            expect(instance.featureFlags.setAnonymousDistinctId).toHaveBeenCalledWith('oldIdentity');
            expect(instance.featureFlags.reloadFeatureFlags).toHaveBeenCalled();
            expect(instance.featureFlags.setPersonPropertiesForFlags).toHaveBeenCalledWith({ email: 'john@example.com', howOftenAmISet: 'once!' }, false);
        });
