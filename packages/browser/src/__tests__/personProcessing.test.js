...
jest.mock('../utils/globals', () => {
    ...
    return {
        ...
        document: {
            ...orig.document,
            createElement: (...args) => orig.document.createElement(...args),
            body: {},
            get referrer() {
...
describe('person processing', () => {
    ...
    const setup = async (person_profiles, token, persistence_name) => {
        token = token || uuidv7();
    ...
    describe('identify', () => {
        it('should fail if process_person is set to never', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('posthog.identify was called, but process_person is set to "never". This call will be ignored.');
        ...
        it('should preserve initial referrer info across subdomain', async () => {
            ...
            mockURLGetter.mockReturnValue('https://example1.com/pathname1?utm_source=foo1');
            // arrange
            const { posthog: posthog1, beforeSendMock: beforeSendMock1 } = await setup('identified_only', undefined, persistenceName);
            // act
            // subdomain 1
            ...
            mockReferrerGetter.mockReturnValue('https://referrer2.com');
            mockURLGetter.mockReturnValue('https://example2.com/pathname2?utm_source=foo2');
            const { posthog: posthog2, beforeSendMock: beforeSendMock2 } = await setup('identified_only', undefined, persistenceName);
    ...
    describe('group', () => {
        ...
        it('should not send the $groupidentify event if person_processing is set to never', async () => {
            ...
            // assert
            expect(mockLogger.error).toBeCalledTimes(1);
            expect(mockLogger.error).toHaveBeenCalledWith('posthog.group was called, but process_person is set to "never". This call will be ignored.');
    ...
    describe('setPersonProperties', () => {
        it("should not send a $set event if process_person is set to 'never'", async () => {
            ...
            expect(beforeSendMock).toBeCalledTimes(0);
            expect(mockLogger.error).toBeCalledTimes(1);
            expect(mockLogger.error).toHaveBeenCalledWith('posthog.setPersonProperties was called, but process_person is set to "never". This call will be ignored.');
        });
    ...
    describe('alias', () => {
        ...
        it('should not send a $create_alias event if person processing is set to "never"', async () => {
            ...
            expect(beforeSendMock).toBeCalledTimes(0);
            expect(mockLogger.error).toBeCalledTimes(1);
            expect(mockLogger.error).toHaveBeenCalledWith('posthog.alias was called, but process_person is set to "never". This call will be ignored.');
        });
    });
    ...
    describe('persistence', () => {
        it('should remember that a user set the mode to always on a previous visit', async () => {
            // arrange
            const persistenceName = uuidv7();
            const { posthog: posthog1, beforeSendMock: beforeSendMock1 } = await setup('always', undefined, persistenceName);
        ...
        it('should work when always is set on a later visit', async () => {
            ...
            const { posthog: posthog1, beforeSendMock: beforeSendMock1 } = await setup('identified_only', undefined, persistenceName);
            posthog1.capture('custom event 1');
            const { posthog: posthog2, beforeSendMock: beforeSendMock2 } = await setup('always', undefined, persistenceName);
            // act
            posthog2.capture('custom event 2');
            // assert
            expect(beforeSendMock1.mock.calls.length).toEqual(1);
    ...
    describe('property calls deduplication', () => {
        it('should deduplicate identical consecutive calls to setPersonProperties', async () => {
            ...
            expect(beforeSendMock).toHaveBeenCalledWith(expect.objectContaining({
                event: '$set',
