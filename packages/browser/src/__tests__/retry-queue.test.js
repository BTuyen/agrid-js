...
describe('RetryQueue', () => {
    ...
    let now = Date.now();
    beforeEach(() => {
        retryQueue = new RetryQueue(mockPosthog);
        jest.useFakeTimers();
    ...
    const enqueueRequests = () => {
        mockPosthog._send_request.mockImplementation(({ callback }) => {
            // Force a retry
            callback === null || callback === void 0 ? void 0 : callback({ statusCode: 502 });
        });
        ...
        mockPosthog._send_request.mockImplementation(({ callback }) => {
            callback === null || callback === void 0 ? void 0 : callback({ statusCode: 200 });
        });
    ...
    it('only calls the callback when successful', () => {
        const cb = jest.fn();
        mockPosthog._send_request.mockImplementation(({ callback }) => {
            callback === null || callback === void 0 ? void 0 : callback({ statusCode: 500 });
        });
        retryQueue.retriableRequest({
            url: '/e',
            data: { event: 'maxretries', timestamp: now },
        ...
        mockPosthog._send_request.mockImplementation(({ callback }) => {
            callback === null || callback === void 0 ? void 0 : callback({ statusCode: 200, text: 'it worked!' });
        });
        fastForwardTimeAndRunTimer();
        expect(retryQueue.length).toEqual(0);
