...
describe('posthog-surveys', () => {
    describe('PostHogSurveys Class', () => {
        let mockPostHog;
        let surveys;
        let mockGenerateSurveys;
        let mockLoadExternalDependency;
        const survey = {
            id: 'completed-survey',
        ...
        beforeEach(() => {
            ...
            surveys = new PostHogSurveys(mockPostHog);
        ...
        describe('canRenderSurvey', () => {
            ...
            it('should return visible: true if surveys are loaded and the survey is eligible', () => {
                ...
                surveys['_surveyManager'] = new SurveyManager(mockPostHog);
        ...
        describe('checkSurveyEligibility', () => {
            ...
            it('cannot render survey if internal_targeting_feature_flag is false', () => {
                ...
                const result = surveys['_checkSurveyEligibility'](survey.id);
                expect(result.eligible).toBeFalsy();
                expect(result.reason).toEqual('Survey internal targeting flag is not enabled and survey cannot activate repeatedly and survey is not in progress');
            });
            ...
            it('can render a survey that is in progress', () => {
                ...
                flagsResponse.featureFlags[survey.linked_flag_key] = true;
                flagsResponse.featureFlags[survey.internal_targeting_flag_key] = false;
                localStorage.setItem(`${SURVEY_IN_PROGRESS_PREFIX}${survey.id}`, JSON.stringify({
            ...
            it('cannot render external surveys', () => {
                ...
                expect(result.eligible).toBeFalsy();
                expect(result.reason).toEqual('Surveys of type external_survey are never eligible to be shown in the app');
            });
            ...
            describe('check order and interaction between multiple eligibility criteria', () => {
                const surveyWithBothConditions = {
                    ...survey,
                ...
                it('allows surveys that pass all checks including repeatability', () => {
                    ...
                    const repeatableSurveyWithWaitPeriod = {
                        ...repeatableSurvey,
        ...
        describe('loadIfEnabled', () => {
            ...
            it('should not load surveys in cookieless mode without consent', () => {
                ...
                const mockIsOptedOut = mockPostHog.consent.isOptedOut;
        ...
        describe('getSurveys', () => {
            ...
            it('should reset _isFetchingSurveys after successful API call', () => {
                ...
                expect((_a = mockPostHog.persistence) === null || _a === void 0 ? void 0 : _a.register).toHaveBeenCalledWith({ [SURVEYS]: mockSurveys });
            });
            ...
            it('should handle delayed successful responses correctly', () => {
                ...
                expect((_a = mockPostHog.persistence) === null || _a === void 0 ? void 0 : _a.register).toHaveBeenCalledWith({ [SURVEYS]: delayedSurveys });
            });
            it('should set correct timeout value in request', () => {
                surveys.getSurveys(mockCallback);
                expect(mockPostHog._send_request).toHaveBeenCalledWith(expect.objectContaining({
