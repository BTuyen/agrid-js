...
describe('utils', () => {
    describe('_.copyAndTruncateStrings', () => {
        ...
        it('handles recursive objects', () => {
            const recursiveObject = { key: 'vaaaaalue', values: ['fooobar'] };
    ...
    describe('_.info', () => {
        it('deviceType', () => {
            const deviceTypes = {
                // iPad
                'Mozilla/5.0 (iPad; CPU OS 6_0 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10A5355d Safari/8536.25': 'Tablet',
                // Samsung tablet
                'Mozilla/5.0 (Linux; Android 7.1.1; SM-T555 Build/NMF26X; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.96 Safari/537.36': 'Tablet',
                // Windows Chrome
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.157 Safari/537.36': 'Desktop',
                // Mac Safari
                'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.75.14 (KHTML, like Gecko) Version/7.0.3 Safari/7046A194A': 'Desktop',
                // iPhone
                'Mozilla/5.0 (iPhone; CPU iPhone OS 13_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.4 Mobile/15E148 Safari/604.1': 'Mobile',
                // LG Android
                'Mozilla/5.0 (Linux; Android 6.0; LG-H631 Build/MRA58K) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/38.0.2125.102 Mobile Safari/537.36': 'Mobile',
            };
    ...
    describe('isLikelyBot', () => {
        it.each(DEFAULT_BLOCKED_UA_STRS.concat('testington'))('blocks a bot based on the user agent %s', (botString) => {
            const randomisedUserAgent = userAgentFor(botString);
            expect(isLikelyBot({ userAgent: randomisedUserAgent }, ['testington'])).toBe(true);
        });
        it.each([
            [
            ...
            expect(isLikelyBot({ userAgent: botString }, [])).toBe(true);
            expect(isLikelyBot({ userAgent: botString.toLowerCase() }, [])).toBe(true);
            expect(isLikelyBot({ userAgent: botString.toUpperCase() }, [])).toBe(true);
        });
        it.each([
            ...
            expect(isLikelyBot({ userAgent: userAgent.toLowerCase() }, [])).toBe(false);
            expect(isLikelyBot({ userAgent: userAgent.toUpperCase() }, [])).toBe(false);
        });
        it('blocks based on the webdriver property being set to true', () => {
            expect(isLikelyBot({ webdriver: true }, [])).toBe(true);
        });
        it('blocks based on userAgentData', () => {
            const headlessUserAgentData = {
                brands: [
        ...
        it('does not crash if the type of navigatorUAData changes', () => {
            ...
            isLikelyBot({ userAgentData: { brands: ['HeadlessChrome'] } }, []);
            // @ts-expect-error testing invalid data
            isLikelyBot({ userAgentData: { brands: [() => 'HeadlessChrome'] } }, []);
            isLikelyBot({ userAgentData: { brands: () => ['HeadlessChrome'] } }, []);
            isLikelyBot({ userAgentData: 'HeadlessChrome' }, []);
            isLikelyBot({ userAgentData: {} }, []);
            isLikelyBot({ userAgentData: null }, []);
            isLikelyBot({ userAgentData: () => ['HeadlessChrome'] }, []);
            isLikelyBot({ userAgentData: true }, []);
        });
    });
    describe('check for cross domain cookies', () => {
        it.each([
            ...
            expect([hostname, isCrossDomainCookie({ hostname })]).toEqual([
                hostname,
                expectedResult,
            ]);
        });
