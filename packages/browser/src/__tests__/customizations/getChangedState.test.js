...
function createComplexState(depth, breadth, includeArrays = false) {
    const state = {};
...
function modifyStateForDrag(state, modifications = 5) {
...
function measureMultipleExecutions(fn, iterations = 10) {
    const times = [];
    let lastResult;
...
describe('getChangedState', () => {
    describe('performance tests', () => {
        ...
        test('should handle large complex state objects (stress test)', () => {
            ...
            const { avgTime, medianTime, stdDev } = measureMultipleExecutions(() => getChangedState(prevState, nextState, 5));
        ...
        test('should handle complex state changes efficiently', () => {
            ...
            const { avgTime, medianTime, stdDev } = measureMultipleExecutions(() => getChangedState(prevState, nextState));
