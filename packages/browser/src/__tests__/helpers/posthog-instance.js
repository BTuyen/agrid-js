...
export const createPosthogInstance = async (token = uuidv7(), config = {}) => {
    ...
    return await new Promise((resolve) => posthog.init(
    ...
    token, {
        ...
        loaded: (p) => {
            var _a;
            (_a = config.loaded) === null || _a === void 0 ? void 0 : _a.call(config, p);
            resolve(p);
        },
    }, 'test-' + token));
};
const posthog = init_as_module();
export const defaultPostHog = () => {
    return posthog;
};
