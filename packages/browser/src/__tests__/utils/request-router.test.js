...
describe('request-router', () => {
    const router = (api_host = 'https://app.posthog.com', ui_host) => {
    ...
    it.each(testCases)('should create the appropriate endpoints for host %s and target %s', (host, target, expectation) => {
    ...
    it('should use the ui_host if provided', () => {
        expect(router('https://my.domain.com/', 'https://eu.posthog.com/').endpointFor('ui')).toEqual('https://eu.posthog.com');
        expect(router('https://my.domain.com/', 'https://app.posthog.com/').endpointFor('ui')).toEqual('https://us.posthog.com');
    });
    it('should react to config changes', () => {
        const mockPostHog = { config: { api_host: 'https://app.posthog.com' } };
        const router = new RequestRouter(mockPostHog);
