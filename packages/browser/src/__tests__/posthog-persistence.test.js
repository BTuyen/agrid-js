...
import { localPlusCookieStore, resetLocalStorageSupported, resetSessionStorageSupported, sessionStore, } from '../storage';
import { defaultPostHog } from './helpers/posthog-instance';
let referrer = ''; // No referrer by default
Object.defineProperty(document, 'referrer', { get: () => referrer });
function makePostHogConfig(name, persistenceMode) {
    return {
        name,
...
describe('persistence', () => {
    let library;
    afterEach(() => {
        library === null || library === void 0 ? void 0 : library.clear();
        document.cookie = '';
        referrer = '';
    });
    const persistenceModes = ['cookie', 'localStorage', 'localStorage+cookie'];
    ...
    describe('localStorage+cookie', () => {
        ...
        it('should migrate data from cookies to localStorage', () => {
            ...
            expect(document.cookie).toContain('ph__posthog=%7B%22distinct_id%22%3A%22testy%22%2C%22test_prop%22%3A%22test_value%22%7D');
        ...
        it(`should additionally store certain values in cookies if localStorage+cookie`, () => {
            ...
            expect(document.cookie).toContain(`ph__posthog=${encode({
                distinct_id: 'test',
            })}`);
