...
describe('posthog core', () => {
    ...
    describe('capture()', () => {
        ...
        it('captures when time is overriden by caller', () => {
            const captureData = posthogWith(defaultConfig, defaultOverrides).capture(eventName, {}, { timestamp: new Date(2020, 0, 2, 12, 34) });
            expect(captureData).toHaveProperty('timestamp');
        ...
        it('handles recursive objects', () => {
            const props = {};
            props.recurse = props;
            expect(() => posthogWith(defaultConfig, defaultOverrides).capture(eventName, props, {
        ...
        it('calls callbacks added via _addCaptureHook', () => {
            ...
            expect(hook).toHaveBeenCalledWith('$event', expect.objectContaining({
                event: '$event',
            }));
        });
        ...
        it('passes through $set and $set_once into the request, if the event is an $identify event', () => {
            ...
            // with them.
            const posthog = posthogWith(defaultConfig, defaultOverrides);
            const captureResult = posthog.capture('$identify', { distinct_id: 'some-distinct-id' }, { $set: { email: 'john@example.com' }, $set_once: { howOftenAmISet: 'once!' } });
            // We assume that the returned result is the object we would send to the
            // server.
            expect(captureResult).toEqual(expect.objectContaining({
        ...
        it('sends payloads to /e/ by default', () => {
            ...
            expect(posthog._send_request).toHaveBeenCalledWith(expect.objectContaining({
                url: 'https://us.i.posthog.com/e/',
            }));
        });
        it('sends payloads to alternative endpoint if given', () => {
            const posthog = posthogWith({ ...defaultConfig, request_batching: false }, defaultOverrides);
            posthog._onRemoteConfig({ analytics: { endpoint: '/i/v0/e/' } });
            posthog.capture('event-name', { foo: 'bar', length: 0 });
            expect(posthog._send_request).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('_afterFlagsResponse', () => {
        ...
        it('enables compression from flags response when only one received', () => {
            const posthog = posthogWith({});
            posthog._onRemoteConfig({ supportedCompression: ['base64'] });
    ...
    describe('_calculate_event_properties()', () => {
        ...
        it("can't deny or blacklist $process_person_profile", () => {
            ...
            expect(posthog.calculateEventProperties('custom_event', { event: 'prop' }, new Date(), uuid)['$process_person_profile']).toEqual(false);
        });
        ...
        it('adds page title to $pageview', () => {
            document.title = 'test';
            expect(posthog.calculateEventProperties('$pageview', {}, new Date(), uuid)).toEqual(expect.objectContaining({ title: 'test' }));
        });
    ...
    describe('bootstrapping feature flags', () => {
        it('sets the right distinctID', () => {
            ...
            expect(posthog.capture).toHaveBeenCalledWith('$identify', {
                distinct_id: 'efgh',
        ...
        it('does nothing when empty', () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('getFeatureFlag for key "multivariant" failed'));
            expect(posthog.getFeatureFlag('disabled')).toBe(undefined);
            expect(posthog.getFeatureFlag('undef')).toBe(undefined);
    ...
    describe('init()', () => {
        ...
        describe('device id behavior', () => {
            ...
            it('sets a random UUID as distinct_id/$device_id if distinct_id is unset', () => {
                ...
                const posthog = uninitialisedPostHog.init(uuidv7(), {
    ...
    describe('group()', () => {
        let posthog;
        beforeEach(() => {
            posthog = defaultPostHog().init('testtoken', {
                persistence: 'memory',
            }, uuidv7());
    ...
    describe('config migration', () => {
        ...
        it('falls back to advanced_disable_decide with deprecation warning', () => {
            ...
            expect(posthog._shouldDisableFlags()).toBe(true);
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("Config field 'advanced_disable_decide' is deprecated"));
        });
    ...
    describe('capturing pageviews', () => {
        it('captures not capture pageview if disabled', async () => {
            ...
            expect(instance.capture).not.toHaveBeenLastCalledWith('$pageview', { title: 'test' }, { send_instantly: true });
        });
    ...
    describe('session_id', () => {
        ...
        it('returns the replay URL', () => {
            expect(instance.get_session_replay_url()).toEqual(`https://us.posthog.com/project/${token}/replay/sessionId`);
        });
        it('returns the replay URL including timestamp', () => {
            expect(instance.get_session_replay_url({ withTimestamp: true })).toEqual(`https://us.posthog.com/project/${token}/replay/sessionId?t=20` // default lookback is 10 seconds
            );
            expect(instance.get_session_replay_url({ withTimestamp: true, timestampLookBack: 0 })).toEqual(`https://us.posthog.com/project/${token}/replay/sessionId?t=30`);
        });
    });
