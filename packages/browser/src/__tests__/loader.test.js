...
describe(`Module-based loader in Node env`, () => {
    ...
    it('should load and capture the pageview event', () => {
        ...
        sinon.assert.calledOnce(posthog.capture);
    ...
    it(`always returns posthog from init`, () => {
        ...
        expect(console.error).toHaveBeenCalledWith('[PostHog.js]', 'PostHog was initialized without a token. This likely indicates a misconfiguration. Please check the first argument passed to posthog.init()');
        // Already loaded logged even when not debug
        expect(posthog.init(`my-test`, { disable_surveys: true }, 'sdk-1')).toBeInstanceOf(PostHog);
        expect(console.warn).toHaveBeenCalledWith('[PostHog.js]', 'You have already initialized PostHog! Re-initializing is a no-op');
    });
});
