...
describe('consentManager', () => {
    const createPostHog = async (config = {}) => {
        const posthog = await new Promise((resolve) => defaultPostHog().init('testtoken', { ...config, loaded: (posthog) => resolve(posthog) }, uuidv7()));
        posthog.debug();
        return posthog;
    };
    let posthog;
    ...
    it('should start default opted in', () => {
        ...
        expect((_a = posthog.persistence) === null || _a === void 0 ? void 0 : _a._disabled).toBe(false);
        expect((_b = posthog.sessionPersistence) === null || _b === void 0 ? void 0 : _b._disabled).toBe(false);
    });
    ...
    it('should start default opted out if setting given and disable storage', async () => {
        ...
        expect(posthog.has_opted_out_capturing()).toBe(true);
        expect(posthog.get_explicit_consent_status()).toBe('pending');
        expect((_a = posthog.persistence) === null || _a === void 0 ? void 0 : _a._disabled).toBe(true);
        expect((_b = posthog.sessionPersistence) === null || _b === void 0 ? void 0 : _b._disabled).toBe(true);
    });
    it('should enable or disable persistence when changing opt out status', async () => {
        ...
        expect(posthog.has_opted_in_capturing()).toBe(true);
        expect((_b = posthog.persistence) === null || _b === void 0 ? void 0 : _b._disabled).toBe(false);
        expect(posthog.get_explicit_consent_status()).toBe('granted');
        posthog.opt_out_capturing();
        expect(posthog.has_opted_in_capturing()).toBe(false);
        expect((_c = posthog.persistence) === null || _c === void 0 ? void 0 : _c._disabled).toBe(true);
    ...
    describe.each([`cookie`, `localStorage`])(`%s`, (persistenceType) => {
        function assertPersistenceValue(value, persistencePrefix = DEFAULT_PERSISTENCE_PREFIX) {
