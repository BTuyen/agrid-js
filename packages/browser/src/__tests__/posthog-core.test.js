...
describe('posthog core', () => {
    ...
    describe('posthog debug logging', () => {
        ...
        it('log when setting debug to undefined', () => {
            const posthog = defaultPostHog().init(uuidv7(), { debug: false });
            posthog.debug();
            expect(console.log).toHaveBeenCalledWith("You're now in debug mode. All calls to PostHog will be logged in your console.\nYou can disable this with `posthog.debug(false)`.");
        });
    ...
    describe('capture()', () => {
        ...
        const setup = (config = {}, token = uuidv7()) => {
        ...
        describe('rate limiting', () => {
            ...
            it('does not capture if rate limit is in place', () => {
                ...
                expect(console.error).toHaveBeenCalledWith('[PostHog.js]', 'This capture call is ignored due to client rate limiting.');
            });
        });
        describe('referrer', () => {
            ...
            it('should not update the referrer within the same session', () => {
                ...
                // assert
                expect(posthog2.persistence.props.$initial_person_info.r).toEqual('https://referrer1.example.com/some/path');
