...
import { SurveyManager, generateSurveys, renderFeedbackWidgetPreview, renderSurveysPreview, useHideSurveyOnURLChange, usePopupVisibility, } from '../../extensions/surveys';
import { retrieveSurveyShadow } from '../../extensions/surveys/surveys-extension-utils';
import { SurveyQuestionType, SurveySchedule, SurveyType, SurveyWidgetType } from '../../posthog-surveys-types';
...
describe('usePopupVisibility', () => {
    ...
    test('should handle multiple surveys with overlapping conditions', () => {
        jest.useFakeTimers();
        const mockSurvey2 = { ...mockSurvey, id: 'testSurvey2', name: 'Test survey 2' };
        const { result: result1 } = renderHook(() => usePopupVisibility(mockSurvey, mockPostHog, 0, false, removeSurvey));
        const { result: result2 } = renderHook(() => usePopupVisibility(mockSurvey2, mockPostHog, 500, false, removeSurvey));
...
describe('SurveyManager', () => {
    ...
    test('callSurveysAndEvaluateDisplayLogic should handle a single popover survey correctly', () => {
        ...
        const handlePopoverSurveyMock = jest
            .spyOn(surveyManager, 'handlePopoverSurvey')
            .mockImplementation(() => { });
    ...
    it('should only display one popover survey if multiple popovers are eligible', () => {
        ...
        const handlePopoverSurveySpy = jest.spyOn(surveyManager, 'handlePopoverSurvey');
        const addSurveyToFocusSpy = jest.spyOn(surveyManager, '_addSurveyToFocus');
    ...
    test('callSurveysAndEvaluateDisplayLogic should handle popup surveys correctly', () => {
        const handlePopoverSurveyMock = jest
            .spyOn(surveyManager, 'handlePopoverSurvey')
            .mockImplementation(() => { });
        const handleWidgetMock = jest.spyOn(surveyManager, '_handleWidget').mockImplementation(() => { });
        const manageWidgetSelectorListener = jest
            .spyOn(surveyManager, '_manageWidgetSelectorListener')
            .mockImplementation(() => { });
        surveyManager.callSurveysAndEvaluateDisplayLogic();
        expect(mockPostHog.surveys.getSurveys).toHaveBeenCalled();
        expect(handlePopoverSurveyMock).toHaveBeenCalledWith(mockSurveys[0]);
    ...
    test('handleWidget should render the widget correctly', () => {
        ...
        const handleWidgetSpy = jest.spyOn(surveyManager, '_handleWidget');
    ...
    test('manageWidgetSelectorListener should be called for selector widgets', () => {
        ...
        const manageWidgetSelectorListenerSpy = jest.spyOn(surveyManager, '_manageWidgetSelectorListener');
        surveyManager.callSurveysAndEvaluateDisplayLogic();
    ...
    test('sortSurveysByAppearanceDelay should sort surveys in progress first', () => {
        ...
        localStorage.setItem(`${SURVEY_IN_PROGRESS_PREFIX}1`, JSON.stringify({
    ...
    describe('timeout management', () => {
        ...
        beforeEach(() => {
            ...
            jest.spyOn(surveyManager, '_addSurveyToFocus');
            jest.spyOn(surveyManager, '_removeSurveyFromFocus');
            // Mock doesSurveyUrlMatch to always return true, used in handlePopoverSurvey
            jest.spyOn(surveyManager, 'handlePopoverSurvey').mockImplementation((survey) => {
                var _a;
    ...
    describe('checkFlags', () => {
        ...
        it('should return false when any feature flag is disabled', () => {
            ...
            jest.spyOn(mockPostHog.featureFlags, 'isFeatureEnabled').mockImplementation((flag) => flag === 'flag-1' ? true : false);
            const result = surveyManager.getTestAPI().checkFlags(survey);
...
describe('usePopupVisibility URL changes should hide surveys accordingly', () => {
    ...
    const createTestSurvey = (urlCondition) => ({
        id: 'test-survey',
        name: 'Test Survey',
...
describe('useHideSurveyOnURLChange', () => {
    ...
    it('should not do anything in preview mode', () => {
        ...
        renderHook(() => useHideSurveyOnURLChange({
            survey,
    ...
    it('should clean up event listeners and history methods on unmount', () => {
        ...
        const { unmount } = renderHook(() => useHideSurveyOnURLChange({
            survey,
            removeSurveyFromFocus: mockRemoveSurveyFromFocus,
            setSurveyVisible: mockSetSurveyVisible,
            isPreviewMode: false,
...
describe('preview renders', () => {
    ...
    test('renderSurveysPreview', () => {
        ...
        const surveyDiv = document.createElement('div');
        expect(surveyDiv.innerHTML).toBe('');
        renderSurveysPreview({ survey: mockSurvey, parentElement: surveyDiv, previewPageIndex: 0 });
    ...
    test('renderFeedbackWidgetPreview', () => {
        ...
        renderFeedbackWidgetPreview({ survey: mockSurvey, root });
    ...
    test('renderSurveysPreview navigates between questions when submitting answers in preview', async () => {
        ...
        console.log('Submit button text:', submitButton === null || submitButton === void 0 ? void 0 : submitButton.textContent);
        await act(async () => {
