...
import { CONSOLE_LOG_RECORDING_ENABLED_SERVER_SIDE, SESSION_RECORDING_ENABLED_SERVER_SIDE, SESSION_RECORDING_IS_SAMPLED, SESSION_RECORDING_REMOTE_CONFIG, } from '../../../constants';
import { SessionIdManager } from '../../../sessionid';
import { FULL_SNAPSHOT_EVENT_TYPE, INCREMENTAL_SNAPSHOT_EVENT_TYPE, META_EVENT_TYPE, } from '../../../extensions/replay/external/sessionrecording-utils';
...
import { LazyLoadedSessionRecording, RECORDING_IDLE_THRESHOLD_MS, RECORDING_MAX_EVENT_SIZE, } from '../../../extensions/replay/external/lazy-loaded-session-recorder';
...
const createStyleSnapshot = (event = {}) => ({
...
const createIncrementalMutationEvent = (mutations) => {
    const mutationData = {
        texts: (mutations === null || mutations === void 0 ? void 0 : mutations.texts) || [],
        attributes: [],
        removes: [],
        adds: [],
        isAttachIframe: true,
...
const createIncrementalStyleSheetEvent = (mutations) => {
    return createIncrementalSnapshot({
        data: {
...
const createCustomSnapshot = (event = {}, payload = {}, tag = 'custom') => ({
...
const createPluginSnapshot = (event = {}) => ({
...
function makeFlagsResponse(partialResponse) {
    return partialResponse;
}
const originalLocation = window.location;
function fakeNavigateTo(href) {
...
describe('Lazy SessionRecording', () => {
    ...
    let sessionManager;
    let config;
    let sessionIdGeneratorMock;
    let windowIdGeneratorMock;
    let onFeatureFlagsCallback;
    let removePageviewCaptureHookMock;
    ...
    beforeEach(() => {
        ...
        sessionManager = new SessionIdManager({ config, persistence: postHogPersistence, register: jest.fn() }, sessionIdGeneratorMock, windowIdGeneratorMock);
        ...
        posthog = {
            get_property: (property_key) => {
                return postHogPersistence === null || postHogPersistence === void 0 ? void 0 : postHogPersistence.props[property_key];
            },
            config: config,
            capture: jest.fn(),
        ...
        // defaults
        (_a = posthog.persistence) === null || _a === void 0 ? void 0 : _a.register({
    ...
    describe('after remote cofig', () => {
        ...
        describe('isConsoleLogCaptureEnabled', () => {
            ...
            ])('%s', (_name, serverSide, clientSide, expected) => {
                var _a;
                (_a = posthog.persistence) === null || _a === void 0 ? void 0 : _a.register({ [CONSOLE_LOG_RECORDING_ENABLED_SERVER_SIDE]: serverSide });
        ...
        describe('is canvas enabled', () => {
            ...
            ])('%s', (_name, fps, quality, expectedFps, expectedQuality) => {
                var _a;
                (_a = posthog.persistence) === null || _a === void 0 ? void 0 : _a.register({
                    [SESSION_RECORDING_REMOTE_CONFIG]: {
        ...
        describe('network timing capture config', () => {
            it.each([
                ...
                expect((_b = sessionRecording['_lazyLoadedSessionRecording']['_networkPayloadCapture']) === null || _b === void 0 ? void 0 : _b.recordPerformance).toBe(expected);
            });
        });
        describe('masking config', () => {
            it.each([
            ...
            ])('%s', (_name, serverConfig, clientConfig, expected) => {
                var _a;
                (_a = posthog.persistence) === null || _a === void 0 ? void 0 : _a.register({
                    [SESSION_RECORDING_REMOTE_CONFIG]: {
                ...
                posthog.config.session_recording.maskAllInputs = clientConfig === null || clientConfig === void 0 ? void 0 : clientConfig.maskAllInputs;
                posthog.config.session_recording.maskTextSelector = clientConfig === null || clientConfig === void 0 ? void 0 : clientConfig.maskTextSelector;
                posthog.config.session_recording.blockSelector = clientConfig === null || clientConfig === void 0 ? void 0 : clientConfig.blockSelector;
        ...
        describe('idle timeouts', () => {
            ...
            function emitInactiveEvent(activityTimestamp, expectIdle = false) {
                const snapshotEvent = {
                    event: 123,
                    type: INCREMENTAL_SNAPSHOT_EVENT_TYPE,
                    data: {
            ...
            function emitActiveEvent(activityTimestamp, expectedMatchingActivityTimestamp = true) {
                const snapshotEvent = {
                    event: 123,
                    type: INCREMENTAL_SNAPSHOT_EVENT_TYPE,
                    data: {
                ...
                if (expectedMatchingActivityTimestamp) {
                    expect(sessionRecording['_lazyLoadedSessionRecording']['_lastActivityTimestamp']).toEqual(activityTimestamp);
                }
                return snapshotEvent;
            }
            beforeEach(() => {
            ...
            it('does not emit plugin events when idle', () => {
                ...
                sessionRecording.onRRwebEmit(createPluginSnapshot({}));
            ...
            it('active incremental events return from idle', () => {
                ...
                // buffer is empty
                expect(sessionRecording['_lazyLoadedSessionRecording']['_buffer']).toEqual(emptyBuffer);
                sessionRecording.onRRwebEmit(createIncrementalSnapshot({}));
            ...
            it('does not emit buffered custom events while idle even when over buffer max size', () => {
                ...
                sessionRecording.onRRwebEmit(createCustomSnapshot({}));
            ...
            it('drops full snapshots when idle - so we must make sure not to take them while idle!', () => {
                ...
                sessionRecording.onRRwebEmit(createFullSnapshot({}));
                expect(sessionRecording['_lazyLoadedSessionRecording']['_buffer']).toEqual({
                    data: [],
                    sessionId: sessionId,
                    size: 0,
            ...
            it('does not emit meta snapshot events when idle - so we must make sure not to take them while idle!', () => {
                ...
                sessionRecording.onRRwebEmit(createMetaSnapshot({}));
                expect(sessionRecording['_lazyLoadedSessionRecording']['_buffer']).toEqual({
                    data: [],
                    sessionId: sessionId,
                    size: 0,
            ...
            it('does not emit style snapshot events when idle - so we must make sure not to take them while idle!', () => {
                ...
                sessionRecording.onRRwebEmit(createStyleSnapshot({}));
                expect(sessionRecording['_lazyLoadedSessionRecording']['_buffer']).toEqual({
                    data: [],
                    sessionId: sessionId,
                    size: 0,
            ...
            it.each(['$session_ending', '$session_starting'])('allows %s events when idle', (eventTag) => {
                // force idle state
                sessionRecording['_lazyLoadedSessionRecording']['_isIdle'] = true;
                ...
                const event = createCustomSnapshot({ timestamp: startingTimestamp + 5000 }, { lastActivityTimestamp: startingTimestamp + 100 }, eventTag);
                sessionRecording.onRRwebEmit(event);
            ...
            it.each(['$session_ending', '$session_starting'])('corrects timestamp for %s events when idle', (eventTag) => {
                ...
                const event = createCustomSnapshot({ timestamp: eventRecordedTime }, { lastActivityTimestamp: lastActivityTime }, eventTag);
                sessionRecording.onRRwebEmit(event);
                const bufferedEvent = sessionRecording['_lazyLoadedSessionRecording']['_buffer'].data[0];
            ...
            it("enters idle state within one session if the activity is non-user generated and there's no activity for (RECORDING_IDLE_ACTIVITY_TIMEOUT_MS) 5 minutes", () => {
                ...
                expect(sessionRecording['_lazyLoadedSessionRecording']['_lastActivityTimestamp']).toEqual(firstActivityTimestamp);
                ...
                expect(posthog.capture).toHaveBeenCalledWith('$snapshot', {
                ...
                expect(sessionRecording['_lazyLoadedSessionRecording']['_lastActivityTimestamp']).toEqual(fourthActivityTimestamp);
        ...
        describe('when pageview capture is disabled', () => {
            ...
            it('captures pageview as expected on non-meta event', () => {
                ...
                expect(sessionRecording['_lazyLoadedSessionRecording']['_tryAddCustomEvent']).toHaveBeenCalledWith('$url_changed', {
                    href: 'https://test.com/',
                });
                sessionRecording['_lazyLoadedSessionRecording']['_tryAddCustomEvent'].mockClear();
                _emit(createIncrementalSnapshot({ type: 3 }));
    ...
    describe('recording', () => {
        ...
        it('can emit when there are circular references', () => {
            ...
            const circularObject = { emit: someObject, again: someObject };
        ...
        describe('the session id manager', () => {
            ...
            const emitAtDateTime = (date, source = 1) => _emit({
                event: 123,
                type: INCREMENTAL_SNAPSHOT_EVENT_TYPE,
            ...
            describe('onSessionId Callbacks', () => {
                ...
                it('calls the callback when the session id changes', () => {
                    ...
                    emitAtDateTime(new Date(startingDate.getFullYear(), startingDate.getMonth(), startingDate.getDate(), startingDate.getHours(), startingDate.getMinutes() + 1));
                    const inactivityThresholdLater = new Date(startingDate.getFullYear(), startingDate.getMonth(), startingDate.getDate(), startingDate.getHours(), startingDate.getMinutes() + 32);
            ...
            describe('with a real session id manager', () => {
                ...
                it('restarts recording if the session is rotated because max time has passed', () => {
                    ...
                    emitAtDateTime(startingDate);
                    emitAtDateTime(new Date(startingDate.getFullYear(), startingDate.getMonth(), startingDate.getDate(), startingDate.getHours(), startingDate.getMinutes() + 1));
                    const moreThanADayLater = new Date(startingDate.getFullYear(), startingDate.getMonth(), startingDate.getDate() + 1, startingDate.getHours() + 1);
    ...
    describe('URL blocking', () => {
        ...
        it('only pauses once when sampling determines session should not record', () => {
            ...
            expect(sessionRecording['_lazyLoadedSessionRecording']['_buffer'].data).toHaveLength(0);
            expect(sessionRecording['_lazyLoadedSessionRecording']['_tryAddCustomEvent']).toHaveBeenCalledWith('recording paused', {
                reason: 'url blocker',
            });
            sessionRecording['_lazyLoadedSessionRecording']['_tryAddCustomEvent'].mockClear();
            _emit(createIncrementalSnapshot({ data: { source: 1 } }));
            // regression: to check we've not accidentally got stuck in a pausing loop
            expect(sessionRecording['_lazyLoadedSessionRecording']['_tryAddCustomEvent']).not.toHaveBeenCalledWith('recording paused', {
                reason: 'url blocker',
            });
        });
    });
    ...
    describe('startIfEnabledOrStop', () => {
        beforeEach(() => {
            ...
            jest.spyOn(sessionRecording, '_lazyLoadAndStart');
        ...
        it('call _lazyLoadAndStart if its enabled', () => {
            ...
            expect(sessionRecording._lazyLoadAndStart).toHaveBeenCalled();
        });
    ...
    describe('sampling', () => {
        ...
        it('turning sample rate to null, means sessions are no longer sampled out', () => {
            ...
            sessionRecording.onRemoteConfig(makeFlagsResponse({ sessionRecording: { endpoint: '/s/', sampleRate: '0.00' } }));
            ...
            sessionRecording.onRemoteConfig(makeFlagsResponse({ sessionRecording: { endpoint: '/s/', sampleRate: null } }));
    ...
    describe('masking', () => {
        it('passes remote masking options to rrweb', () => {
            ...
            expect(assignableWindow.__PosthogExtensions__.rrweb.record).toHaveBeenCalledWith(expect.objectContaining({
                maskAllInputs: true,
                maskTextSelector: '*',
            }));
        });
        describe('capturing passwords', () => {
            it.each([
                ['no masking options', {}, true],
                ['empty masking options', { maskInputOptions: {} }, true],
                ['password not set', { maskInputOptions: { input: true } }, true],
                ['password set to true', { maskInputOptions: { password: true } }, true],
                ['password set to false', { maskInputOptions: { password: false } }, false],
            ])('%s', (_name, session_recording, expected) => {
    ...
    describe('linked flags', () => {
        it('stores the linked flag on flags response', () => {
            sessionRecording.onRemoteConfig(makeFlagsResponse({ sessionRecording: { endpoint: '/s/', linkedFlag: 'the-flag-key' } }));
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlag).toEqual('the-flag-key');
            ...
            onFeatureFlagsCallback === null || onFeatureFlagsCallback === void 0 ? void 0 : onFeatureFlagsCallback(['the-flag-key'], { 'the-flag-key': true });
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlagSeen).toEqual(true);
            expect(sessionRecording.status).toEqual('active');
            onFeatureFlagsCallback === null || onFeatureFlagsCallback === void 0 ? void 0 : onFeatureFlagsCallback(['different', 'keys'], { different: true, keys: true });
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlagSeen).toEqual(false);
            expect(sessionRecording.status).toEqual('buffering');
        });
        it('does not react to flags that are present but false', () => {
            ...
            expect(sessionRecording.status).toEqual('buffering');
            expect(onFeatureFlagsCallback).not.toBeNull();
            onFeatureFlagsCallback === null || onFeatureFlagsCallback === void 0 ? void 0 : onFeatureFlagsCallback(['the-flag-key'], { 'the-flag-key': false });
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlagSeen).toEqual(false);
            expect(sessionRecording.status).toEqual('buffering');
        });
        it('can handle linked flags with variants', () => {
            ...
            expect(sessionRecording.status).toEqual('buffering');
            expect(onFeatureFlagsCallback).not.toBeNull();
            onFeatureFlagsCallback === null || onFeatureFlagsCallback === void 0 ? void 0 : onFeatureFlagsCallback(['the-flag-key'], { 'the-flag-key': 'test-a' });
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlagSeen).toEqual(true);
            expect(sessionRecording.status).toEqual('active');
            onFeatureFlagsCallback === null || onFeatureFlagsCallback === void 0 ? void 0 : onFeatureFlagsCallback(['the-flag-key'], { 'the-flag-key': 'control' });
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlagSeen).toEqual(false);
            expect(sessionRecording.status).toEqual('buffering');
        });
        it('can handle linked flags with any variants', () => {
            ...
            expect(sessionRecording.status).toEqual('buffering');
            expect(onFeatureFlagsCallback).not.toBeNull();
            onFeatureFlagsCallback === null || onFeatureFlagsCallback === void 0 ? void 0 : onFeatureFlagsCallback(['the-flag-key'], { 'the-flag-key': 'literally-anything' });
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlagSeen).toEqual(true);
            expect(sessionRecording.status).toEqual('active');
            onFeatureFlagsCallback === null || onFeatureFlagsCallback === void 0 ? void 0 : onFeatureFlagsCallback(['not-the-flag-key'], { 'not-the-flag-key': 'literally-anything' });
            expect(sessionRecording['_lazyLoadedSessionRecording']['_linkedFlagMatching'].linkedFlagSeen).toEqual(false);
            expect(sessionRecording.status).toEqual('buffering');
        });
        it('can be overriden', () => {
    ...
    describe('when rrweb is not available', () => {
        ...
        it('processes the queue when rrweb is available again', () => {
            addRRwebToWindow();
            sessionRecording['_lazyLoadedSessionRecording'].onRRwebEmit(createIncrementalSnapshot({ data: { source: 1 } }));
            expect(sessionRecording['_lazyLoadedSessionRecording']['_queuedRRWebEvents']).toHaveLength(0);
        });
    });
    ...
    describe('session linking', () => {
        ...
        it('emits session linking events on activity timeout', () => {
            const tryAddCustomEvent = sessionRecording['_lazyLoadedSessionRecording']['_tryAddCustomEvent'];
        ...
        it('includes flushed_size with actual data size in session ending event', () => {
            ...
            const flushedSize = sessionRecording['_lazyLoadedSessionRecording']['_flushedSizeTracker'].currentTrackedSize;
        ...
        it('always emits $session_id_change event regardless of change reason', () => {
            ...
            testCases.forEach((changeReason, index) => {
                ...
                expect(tryAddCustomEvent).toHaveBeenCalledWith('$session_id_change', expect.objectContaining({
                    sessionId: newSessionId,
                    windowId: newWindowId,
                }));
            });
