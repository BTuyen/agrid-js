...
import { allMatchSessionRecordingStatus, AndTriggerMatching, anyMatchSessionRecordingStatus, OrTriggerMatching, } from '../../../extensions/replay/external/triggerMatching';
...
describe('SessionRecording', () => {
    ...
    describe('onRemoteConfig()', () => {
        ...
        it('uses anyMatchSessionRecordingStatus when triggerMatching is "any"', () => {
            ...
            expect(sessionRecording['_lazyLoadedSessionRecording']['_statusMatcher']).toBe(anyMatchSessionRecordingStatus);
            expect(sessionRecording['_lazyLoadedSessionRecording']['_triggerMatching']).toBeInstanceOf(OrTriggerMatching);
        });
        it('uses allMatchSessionRecordingStatus when triggerMatching is "all"', () => {
            sessionRecording.onRemoteConfig(makeFlagsResponse({
                sessionRecording: { endpoint: '/s/', triggerMatchType: 'all' },
            }));
            expect(sessionRecording['_lazyLoadedSessionRecording']['_statusMatcher']).toBe(allMatchSessionRecordingStatus);
            expect(sessionRecording['_lazyLoadedSessionRecording']['_triggerMatching']).toBeInstanceOf(AndTriggerMatching);
        });
        ...
        it('stores true in persistence if recording is enabled from the server', () => {
            var _a;
            (_a = posthog.persistence) === null || _a === void 0 ? void 0 : _a.register({ [SESSION_RECORDING_REMOTE_CONFIG]: undefined });
            sessionRecording.onRemoteConfig(makeFlagsResponse({ sessionRecording: { endpoint: '/s/' } }));
