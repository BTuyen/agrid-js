...
describe('Error conversion', () => {
    ...
    function errorToProperties({ event, error }, hint) {
        return errorPropertiesBuilder.buildFromUnknown(error !== null && error !== void 0 ? error : event, {
            mechanism: {
                handled: hint === null || hint === void 0 ? void 0 : hint.handled,
            },
            syntheticException: hint === null || hint === void 0 ? void 0 : hint.syntheticException,
        });
    }
    it('should convert a string to an error', () => {
        ...
        expect(errorToProperties({ event: 'Uncaught exception: InternalError: but somehow still a string' })).toEqual(expected);
    });
    it('should convert a plain object to an error', () => {
        ...
        expect(errorToProperties({ event: { string: 'candidate', foo: 'bar' } })).toEqual(expected);
    });
    ...
    it('should convert a DOM Error to an error', () => {
        ...
        expect(errorToProperties({ event: event })).toEqual(expected);
    });
    it('should convert a DOM Exception to an error', () => {
        const event = new DOMException('oh no disaster', 'dom-exception');
        const errorProperties = errorToProperties({ event: event });
        if (isNull(errorProperties)) {
            throw new Error("this mustn't be null");
        }
        expect(Object.keys(errorProperties)).toHaveLength(2);
    ...
    it('should convert unhandled promise rejection', () => {
        ...
        const errorProperties = errorToProperties({ event: pre }, { handled: false });
        expect(Object.keys(errorProperties)).toHaveLength(2);
        expect(errorProperties.$exception_list[0].type).toEqual('PromiseRejectionEvent');
        expect(errorProperties.$exception_list[0].value).toEqual('PromiseRejectionEvent captured as exception with keys: isTrusted, promise, reason');
        expect(errorProperties.$exception_level).toEqual('error');
    ...
    it('should use cause when it is an error', () => {
        ...
        const errorProperties = errorToProperties({ error, event: undefined });
        expect(Object.keys(errorProperties)).toHaveLength(2);
        expect(errorProperties.$exception_list[0].type).toEqual('Error');
    ...
    it('should not forward handled prop when using cause', () => {
        ...
        const errorProperties = errorToProperties({ error, event: undefined }, metadata);
        expect(Object.keys(errorProperties)).toHaveLength(2);
        expect(errorProperties.$exception_list.length).toEqual(2);
        expect(errorProperties.$exception_list[0].mechanism.synthetic).toEqual(false);
        expect(errorProperties.$exception_list[0].mechanism.handled).toEqual(false);
