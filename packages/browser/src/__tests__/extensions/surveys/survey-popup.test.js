...
describe('SurveyPopup', () => {
    ...
    let mockOnCloseConfirmationMessage;
    // Type cast mocks for easier usage
    const mockedGetInProgressSurveyState = surveyUtils.getInProgressSurveyState;
    ...
    const mockedSendSurveyEvent = surveyUtils.sendSurveyEvent;
    const mockedDismissedSurveyEvent = surveyUtils.dismissedSurveyEvent;
    const mockedUuidv7 = uuid.uuidv7;
    beforeEach(() => {
        cleanup();
        jest.clearAllMocks();
    ...
    test('calls onCloseConfirmationMessage when X button is clicked in the confirmation message', () => {
        render(_jsx(SurveyPopup, { survey: mockSurvey, removeSurveyFromFocus: mockRemoveSurveyFromFocus, isPopup: true, onCloseConfirmationMessage: mockOnCloseConfirmationMessage, previewPageIndex: mockSurvey.questions.length, posthog: mockPosthog }));
    ...
    test('initializes with new submissionId and empty responses when no state in localStorage', () => {
        mockedGetInProgressSurveyState.mockReturnValue(null);
        render(_jsx(SurveyPopup, { survey: mockSurvey, removeSurveyFromFocus: mockRemoveSurveyFromFocus, isPopup: true, posthog: mockPosthog }));
    ...
    test('saves partial response to localStorage when moving to next question', () => {
        ...
        render(_jsx(SurveyPopup, { survey: partialResponsesSurvey, removeSurveyFromFocus: mockRemoveSurveyFromFocus, isPopup: true, posthog: mockPosthog }));
    ...
    test('always shows external surveys even if millisecondDelay is set', () => {
        ...
        render(_jsx(SurveyPopup, { survey: survey, removeSurveyFromFocus: mockRemoveSurveyFromFocus, isPopup: true, posthog: mockPosthog }));
