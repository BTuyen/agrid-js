...
import { SurveyQuestionType, SurveyType, SurveyWidgetType } from '../../../posthog-surveys-types';
...
const urlConditionWidgetSurvey = {
    ...baseWidgetSurvey,
...
describe('FeedbackWidget', () => {
    ...
    const expectSurveyShowEvent = (surveyId) => {
        expect(mockPosthog.capture).toHaveBeenCalledWith('survey shown', expect.objectContaining({ $survey_id: surveyId }));
    };
    const expectSurveySentEvent = (surveyId, response) => {
        expect(mockPosthog.capture).toHaveBeenLastCalledWith('survey sent', expect.objectContaining({ $survey_id: surveyId, ...response }));
    };
    test('renders feedback tab and opens survey on click', () => {
        render(_jsx(FeedbackWidget, { survey: baseWidgetSurvey, posthog: mockPosthog }));
    ...
    test('hides/shows feedback tab based on URL condition', async () => {
        ...
        render(_jsx(FeedbackWidget, { survey: urlConditionWidgetSurvey, posthog: mockPosthog }));
    ...
    test('does not render tab for selector widget type initially', () => {
        render(_jsx(FeedbackWidget, { survey: selectorWidgetSurvey, posthog: mockPosthog }));
    ...
    test('closes survey popup when cancel button is clicked', async () => {
        ...
        expect(mockPosthog.capture).toHaveBeenCalledWith('survey dismissed', expect.objectContaining({
